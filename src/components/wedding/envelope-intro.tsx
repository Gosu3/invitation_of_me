...
export function EnvelopeIntro({ config, phase, open }: { config: WeddingInvitationConfig; phase: OpeningPhase; open: () => void }) {
  ...
  return <section className={`cover-gate opening-${phase}`} aria-label="Mở thiệp cưới">
    ...
    <article className="cover-card">
      <Image className="cover-flower cover-flower-left" src={theme.assets.flower} alt="" aria-hidden="true" width={360} height={360} preload />
      <Image className="cover-flower cover-flower-right" src={theme.assets.flower} alt="" aria-hidden="true" width={360} height={360} preload />
      <div className="cover-card-content">
        ...
        <button className="cover-open-button" onClick={open} disabled={phase !== 'closed'} aria-busy={phase !== 'closed'}>{phase === 'closed' ? 'Mở thiệp' : 'Đang mở…'} <ArrowRight size={16} /></button>
      </div>
    </article>
  </section>;
}
