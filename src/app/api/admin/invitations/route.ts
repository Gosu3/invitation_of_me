...
export async function GET() {
  ...
  const { data, error } = await admin.service.from('wedding_invitations').select('id,slug,status,admin_title,partner_one,partner_two,cover_media_id,created_at,updated_at,published_at').order('updated_at', { ascending: false });
